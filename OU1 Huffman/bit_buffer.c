 ...
 *
 * Copyright 2020 Jonny Pettersson (jonny@cs.umu.se). Permission is
                              ...
                              const int size)
{
	...
	b->array = realloc(b->array, (size / 8));
	assert(b->array);
...
void bit_buffer_insert_bit(bit_buffer *b, const int value)
{
	...
	if (bit_buffer_size(b) + 1 == b->capacity) {
		...
		if (b->next_remove >= b->next_insert) {
			...
			for (i = b->next_remove + 1 ; i < b->next_remove + 9 ; i++) {
...
char bit_buffer_remove_byte(bit_buffer *b)
{
	...
	assert(b->array);
	assert(b->size >= 8);
	int byte_no = b->next_remove / 8;
	char the_byte = b->array[byte_no];

	b->array[byte_no] = 0;
	b->next_remove = (b->next_remove + 8) % b->capacity;
	b->size -= 8;
...
char *bit_buffer_array_to_string(const bit_buffer *const b)
{
	assert(b);
	assert(b->array);
	char *res = calloc(b->capacity, sizeof(char));
	assert(res);
...
char *bit_buffer_to_string(const bit_buffer *const b)
{
	assert(b);
	assert(b->array);
	char *res = calloc(bit_buffer_size(b), sizeof(char));
	for (int i = 0 ; i < bit_buffer_size(b) ; i++) {
